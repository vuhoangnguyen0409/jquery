        ...
        return this.each(function() {
           ...
           /* for (var i = 0; i < numberOfSlide; i++) {
            ...
            sliderWrapper.find('a.nSliderNumber').click(function() {
                ...
                //alert(href);
                $(this).parent().siblings('.sliderImgs').children().stop().fadeOut();
                $(this).parent().siblings('.sliderImgs').children().eq(href - 1).stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).addClass('showed').siblings().removeClass('showed');
                return false;
            });
            // css control button
            if (obj.control) {
            ...
            sliderWrapper.find('a.next').click(function() {
                if ($(this).parent().siblings('.sliderImgs').children().last().hasClass('showed')) { // last slide
                    $(this).parent().siblings('.sliderImgs').children().first().stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).addClass('showed').siblings().stop().fadeOut().removeClass('showed');
                } else {
                    $showed = $(this).parent().siblings('.sliderImgs').find('.showed');
                    $showed.removeClass('showed').next().addClass('showed').stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).siblings().stop().fadeOut();
                }
                return false;
            });
            sliderWrapper.find('a.prev').click(function() {
                if ($(this).parent().siblings('.sliderImgs').children().first().hasClass('showed')) { // first slide
                    $(this).parent().siblings('.sliderImgs').children().last().stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).addClass('showed').siblings().stop().fadeOut().removeClass('showed');
                } else {
                    $showed = $(this).parent().siblings('.sliderImgs').find('.showed');
                    $showed.removeClass('showed').prev().addClass('showed').stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).siblings().stop().fadeOut();
                }
                return false;
            });
            //auto mode
            if (obj.auto) {
                setInterval(function() {
                    if (slider.children().eq(numberOfSlide - 1).hasClass('showed')) { // last slide
                        slider.children().eq(0).stop().fadeIn(obj.fadeInSpeed).animation(obj.mode).addClass('showed').siblings().stop().fadeOut().removeClass('showed');
                    } else {
           ...
           /* sliderWrapper.find('.maxHeightImg img').clone().appendTo(sliderWrapper).wrap('<div class="clone"></div>').css({
			...
			slider.children().first().find('img').clone().appendTo(sliderWrapper).wrap('<div class="clone"></div>').css({
                'visibility': 'hidden'
            });
            // initial
